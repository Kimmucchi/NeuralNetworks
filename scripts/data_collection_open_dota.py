...
class OpenDotaAPI():
    ...
    def get_recent_matches(self, fetch_num = 100):
        params = dict()
        times_called = math.ceil(fetch_num / 100)
        match_list = []
        sleep = False
        for i in range(times_called):
            params['less_than_match_id'] = self.last_match_id
            url = "https://api.opendota.com/api/publicMatches"
            call_matches = self._call(url, params)
            match_list += call_matches
            try:
                self.last_match_id = min([item['match_id'] for item in call_matches])
                sleep = False
            except:
                if not sleep:
                    time.sleep(0.5)
...
class DataPreprocessing():
    ...
    def to_csv(self):
        self.matches.to_csv("matches5.csv", index=False)
        self.players.to_csv("players5.csv", index=False)
        #self.chat.to_csv("chat.csv")
        #self.objectives.to_csv("objectives.csv")
        #self.advantages.to_csv("advantages.csv")
        #self.events.to_csv("events.csv")
        #self.abilities.to_csv("abilities.csv")
        #self.wards.to_csv("wards.csv")
        #self.previous_matches.to_csv("prev_matches.csv")
...
def main(sleep_time = 1):
    ...
    recent_matches = filter_matches(api.get_recent_matches(fetch_num=20000))
    for i, recent_match in enumerate(recent_matches):
        print(i+1, "/", len(recent_matches))
        try:
...
def _filter_function(match):
    if type(match) != dict:
        return False
    elif match['duration'] < 1000 or match['duration'] > 4200:
        return False
